...
## CEMS module layout (`custom-addons/`)

Apply the principles above inside each CEMS addon:

| Concern | Location |
|---------|----------|
| ORM / business rules | `models/` — `@api.depends`, `@api.constrains`, `UserError` / `ValidationError` |
| HTTP / portal / website | `controllers/` — `@http.route`, try/except, `_logger` |
| Pure helpers (no ORM) | `utils/` |
| UI & QWeb | `views/`, `data/` |
| ACL & record rules | `security/` |
| Automated tests | `tests/` — tag `cems` |

- Never edit Odoo core; only `_inherit` + XML `xpath`.
- Mark user-facing strings with `_()`.
- Keep Phase boundaries (see each module's `AGENTS.md`).
